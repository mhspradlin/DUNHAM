...
// The Dunham telescope control program.  
// Version 1.1

// Barbara Carter  April 12, 1995 
...
// subroutine to perform whatever function the user has chosen.
{
   switch (*menu)
   {
      ...
      case TimeSetMenu:     DoTimeMenu(menu);        break;
      case LocalTimeMenu:   DoLocalTimeMenu(menu);   break;
      case LocalDateMenu:   DoLocalDateMenu(menu);   break;
      case DaylightMenu:    DoDaylightMenu(menu);    break;
      case UniversalTimeMenu: DoUniversalTimeMenu(menu); break;
...
{  
   ...
   gotoxy(31,y+4); 
   if (HaveBeenTracking) printf("1 - Resume Tracking"); 
                    else printf("1 - Start Tracking"); 
   gotoxy(31,y+5);       printf("2 - Set Time");
   gotoxy(31,y+6);       printf("3 - Test Motors");
   gotoxy(31,y+7);       printf("4 - Set Preferences");
   gotoxy(31,y+8);       printf("5 - Exit Program"); 
   gotoxy(29,y+10);
   GetSelection(menu,5);
   switch(*menu)
   {
      case 1: if (HaveBeenTracking) 
                 *menu=TrackingMenu;  
              else *menu=ZenithMenu;
              break;
      case 2: *menu=TimeSetMenu;     break;
      case 3: *menu=MotorMenu;       break;
      case 4: *menu=PreferencesMenu; break;
      case 5: *menu=ExitProgram;     break;
...
// hardware reset (like pressing the reset switch, or turning it off and on).
{
   ...
   for (i=2;i<23;i++) EraseLine(i);
   gotoxy(21,12);
   printf("Thank you for using the Dunham telescope");
   gotoxy(1,24);
   *menu=-1;
}
...
// the ports and ISR's that will be needed.
{
   ...
   if (fileptr)
   {
      ...
      if (!err) 
      {
         result = fscanf( fileptr, "%*s %ld", &ldummy ); // MaxRange
         if (result != 1) err=1;
         else MaxRange = ldummy;
      }
      if (!err) 
      {
      ...
      if (!err) 
      {
         result = fscanf( fileptr, "%*s %d", &dummy );  // E_W
         if (result != 1) err=1;
         else E_W = dummy;
      }
      if (!err) 
      {
         result = fscanf( fileptr, "%*s %d", &dummy );  // DaylightSavings
         if (result != 1) err=1;
         else DaylightSavings = dummy;
      }
      fclose( fileptr );
   }
   ...
   outp( CHANNEL0, TRACKLOWORD );  // speed up the clock ticks
   outp( CHANNEL0, TRACKHIWORD );
   setvect(8,tracking_ticks);      // hook the clock-tick ISR.
   enable();
   StartTiming();                  // Sets the Universal Time.
   clrscr();
}
...
//   gotoxy(80,24); printf("%c", 0xBC);
   gotoxy(1,1); putchar(0xC9);
   gotoxy(1,24); putchar(0xC8);
   gotoxy(80,1); putchar(0xBB);
   gotoxy(80,24); putchar(0xBC);
   for (i=2;i<80;i++) 
   {
//      gotoxy(i,24); printf("%c", 0xCD); // draw the bottom line,
      gotoxy(i,24); putchar(0xCD);
   }
   for (i=2;i<title_x;i++)
   {
//      gotoxy(i,1); printf("%c", 0xCD); // and the top line,
      gotoxy(i,1); putchar(0xCD);
   }
...
//      gotoxy(80,i); printf("%c", 0xBA);  // and the right.
      gotoxy(1,i); putchar(0xBA);
      gotoxy(80,i); putchar(0xBA);
   }
   gotoxy(x,y);
}
