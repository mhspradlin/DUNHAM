...
// motors, what is displayed, etc.
{
   ...
   gotoxy(x,y+5); 
   printf("3 - Change Fast paddle maximum speed (%2.0f%%)", 
               100.0*(double)MaxRange/(double)Levels);
   gotoxy(x,y+6); 
   ...
   gotoxy(x,y+10);
   printf("8 - Reverse the East and West paddle buttons ");
   if (E_W>0) printf("(left=West)");
   else printf("(left=East)");
   gotoxy(x,y+11);
   ...
   switch(*menu)
   {
      ...
      case 9: *menu=MainMenu;         
              ...
              if ( MaxRange!=oldMaxRange ) diff=1;       // for Fast Paddle
...
void DoFastPaddleMenu( int *menu )
{
   ...
   *menu = PreferencesMenu;
   clrscr();
   FrameScreen();
   gotoxy(28,8); printf("Fast Paddle Maximum Speed");
   gotoxy(x,10); 
   printf("This sets the maximum speed for the paddle when the upper ");
   printf("right-hand"); 
   gotoxy(x,11);
   printf("black button is switched to 'fast'.  Values too close to 100%%");
   gotoxy(x,12);
   printf("will cause the motors to bind.  If this happens, you must ");
   printf("re-zenith"); 
   gotoxy(x,13);
   printf("the telescope.");
   gotoxy(x,14);
   printf("Use the %c and %c arrow keys to select a faster or slower value.",
         24,25);
   gotoxy(x,16);
   printf("Current setting is: %2.0f%%", 
           100.0*(double)MaxRange/(double)Levels);
   do
   {
      EraseLine(18); EraseLine(19); EraseLine(20); EraseLine(21);
      gotoxy(x,18);
      printf("Use arrow keys.  Hit return when done, to save setting.");
      gotoxy(x,19);
      printf("Fast Paddle Speed: %2.0f%%", 
              100.0*(double)MaxRange/(double)Levels);
      while (!kbhit()) ;
      letter = getch();
      if (letter=='\n' || letter=='\r') 
      ...
      {
         if ( letter==Down && MaxRange>0 ) 
         {
            MaxRange--;
            printf("%c",25);
         }
         if ( letter==Up && MaxRange<Levels ) 
         {
            MaxRange++;
            printf("%c",24);
         }
...
void SavePrefsFile( void )
{
   ...
   if (fileptr)
   {
      ...
      fprintf( fileptr, "E_W %d\n", E_W );
      fprintf( fileptr, "DaylightSavings %d\n", DaylightSavings );
      fclose(fileptr);
   }
}
