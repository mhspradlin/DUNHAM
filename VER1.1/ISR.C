...
// to stop the slew.
{
   ...
   if (Status==TRACKING && scancode>=72 && scancode <=80)
   {
      switch (scancode)
      {
         case 72: UpArrow=1; TimeOfLastKey=TOD; break;
         case 80: DnArrow=1; TimeOfLastKey=TOD; break;
         case 75: LfArrow=1; TimeOfLastKey=TOD; break;
         case 77: RtArrow=1; TimeOfLastKey=TOD; break;
...
// tracking, and updates an internal time-of-day counter, TOD.
{
   ...
   if (Status == TRACKING)  // send out pulses only during tracking.
                            // (During slewing, pulses are sent from a
                            // subroutine, not this clock-tick ISR.)
   {
...
// of clock-ticks when switching between different clock-tick speeds.
{
   if ( ++ticks >= FACTOR )  // if we're on an "old" clock tick...
   {
      ...
      disable();
      setvect( 8, oldclk );         // hook the old clock tick isr,
