...
{
   int               x=27, y=TRACKINGDISPLAY, xx, yy, firstxx, i;
   int               letter, menuset=0, choice, s2=2*ShowXY;
   ...
   do
   {
      ...
      gotoxy(x-8, y+9+s2); printf("1 - Slew to new RA/Dec coordinates");
      gotoxy(x-8,y+10+s2); printf("2 - Change jog amount (%ld arcsec)", Jog_Angle);
      gotoxy(x-8,y+11+s2); printf("3 - Set current coordinates to original values");
      gotoxy(x-8,y+12+s2); printf("4 - Stop tracking");
      gotoxy(x-8,y+13+s2); printf("Arrow keys - jog telescope by %ld arcsec", Jog_Angle);
      gotoxy(x,y+15+s2); printf("Enter your selection: ");
...
//t_->tm_year, t_->tm_mon, t_->tm_mday, t_->tm_hour, t_->tm_min, t_->tm_sec);
      ...
      StartingUT += ( (double)(t_->tm_min) + (double)(t_->tm_sec)/SIXTY )/SIXTY;
      ticks=0;
      TOD=0L;
      PrevStatus = Status;
      Status = TRACKING;
...
// Runs until the user presses a key, grabs it, and returns.
{
   int i=DELAYFACTOR, out_of_bounds=0;
   long int R;
   ...
   {
      TrackCalc();
      if ((++i) > DELAYFACTOR)    // only re-display every so often. 
      {
   ...
   if (!out_of_bounds) *letter = getche();
   else *letter = 0;
}
...
// the paddle-speed button.  0=slow, 1=fast.  Or the other way 'round.
{
